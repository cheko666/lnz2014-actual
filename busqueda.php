...
$titulo = " B&uacute;squeda ".$busqueda;
...
if (empty($_GET['search'])) {
	$Seccion_Subtitulo = " Productos ";
	$mensajeProductoNoEncontrado = "<p class='center'><strong>Usted no escribi&oacute; nada en el buscador.</strong></p>";
}
if (isset($_GET['search']) && $_GET['search']!='') {
	$busqueda="";
	$busqueda=$_GET['search'];
	$busqueda_stemm=stemm_es::stemm($busqueda);
...
mysql_query("SET NAMES 'utf8'");
	...
	WHERE (p.Title LIKE '%$busqueda_stemm%' AND p.Activo=1) OR (m.Title LIKE '%$busqueda_stemm%' AND p.Activo=1)
	GROUP BY p.ID
	...
	if(mysql_num_rows($Productos)>0){
		$mensajeProductoNoEncontrado = "<p class='center'>Resultados encontrados para <strong><big>\"".$busqueda."\"</big></strong></p>";
	}else{ 
...
mysql_query("SET NAMES 'utf8'");
		$insert_sql =
		"INSERT INTO Busqueda_Frases (Frase) SELECT '$busqueda'";
			...
			<div id="contenido">
			  <!-- InstanceBeginEditable name="Contenido" -->
				...
				<div id="productos_grid">
					<?php if(mysql_num_rows($Productos)==0) { echo "<div class='mensaje'>".$mensajeProductoNoEncontrado."</div>";} else { ?>
					<div class='mensaje'><?php echo $mensajeProductoNoEncontrado;?>
					</div>
